...
namespace uns {
	...
	protected:
		static void convert(std::u32string& to, const std::u8string_view& from) {
			auto res = std::u32string(); res.resize(from.size());
			...
			for(const auto& val : from) {
				if(val & 0xE0 == 0xC0) {
					if(degree == 1) {
						...
						symbol += static_cast<char8_t>((val & 0x3F) << 6 * (--degree));
					}
					else
						degree = 1;
				}
					...
					degree = 4;
					symbol += static_cast<char8_t>((val & 0x07) << 6 * (--degree));
				}
				else if(val & 0xF0 == 0xE0) {
					degree = 3;
					symbol += static_cast<char8_t>((val & 0x0F) << 6 * (--degree));
				}
				else if(val & 0xF0 == 0xE0) {
					degree = 3;
					symbol += static_cast<char8_t>((val & 0x0F) << 6 * (--degree));
		...
		static void convert(std::u8string& to, const std::u32string_view& from) {
			...
			for(const auto& val : from) {
				if(val < 0x80) {
					oct1 = static_cast<char8_t>(val);
					res += oct1;
					total += 1;
				}
				else if(val < 0x800) {
					oct1 = 0xC0 + static_cast<char8_t>(val / 0x40);
					oct2 = 0x80 + static_cast<char8_t>(val % 0x40);
					...
					oct1 = 0xE0 + static_cast<char8_t>(val / 0x1000);
					oct2 = 0x80 + static_cast<char8_t>((val % 0x1000) / 0x40);
					oct3 = 0x80 + static_cast<char8_t>(val % 0x40);
					...
					oct1 = 0xF0 + static_cast<char8_t>(val / 0x10000);
					oct2 = 0x80 + static_cast<char8_t>((val % 0x10000) / 0x1000);
					oct3 = 0x80 + static_cast<char8_t>((val % 0x1000) / 0x40);
					oct4 = 0x80 + static_cast<char8_t>(val % 0x40);
			...
			to = std::u8string(res.begin(), res.begin() + total);
		};
