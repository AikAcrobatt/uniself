...
namespace uns {
    ...
    namespace nn {
        ...
        public:
            ...
            virtual string_t String(string_t delimiter, bool return_dynamic_data) = 0;
        ...
        public:
            ...
            std::string Settle(const string_t& str, string_t delimiter, bool set_dynamic_data = false) {
				...
				ctx->Access().errstream.open(ctx->Access().Get(CTX_CSTRING::ErrorFileName), std::ios::binary | std::ios::app);
				if(ctx->Access().errstream.bad()) res += "Opening ErrFile failed;";
