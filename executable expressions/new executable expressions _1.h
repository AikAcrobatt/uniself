...
namespace uns::syntax {
    ...
    public:
        ...
        virtual int& Exec() override { base_t::subnodes[0]->Exec(); int res = 0; return res; };
    };
    ...
    public:
        ...
        executable_tree& operator=(const executable_tree& copying_obj) {
            if(&copying_obj != this) {
                Delete();

                nodes.resize(copying_obj.nodes.size());
                size_t idx = 0;
                for(auto node_ptr : copying_obj.nodes) {
                    *nodes[idx++] = *node_ptr;
                };
                idx = 0;
                for(auto node_ptr : copying_obj.nodes) {
                ...
                variables = std::move(copying_obj.variables);
            };
            return *this;
        };
