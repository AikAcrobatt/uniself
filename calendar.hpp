#pragma once

#include <chrono>

#include "uniself/renum.hpp"

#ifndef UNS_LIB_CALENDAR
#define UNS_LIB_CALENDAR

namespace uns::calendar::gregorian {

    UNS_RENUM(month, uint8_t
        , (january,     = 0)
        , (february,    = 1)
        , (march,       = 2)
        , (april,       = 3)
        , (may,         = 4)
        , (june,        = 5)
        , (july,        = 6)
        , (august,      = 7)
        , (september,   = 8)
        , (october,     = 9)
        , (november,    = 10)
        , (december,    = 11)
    );


    UNS_RENUM(day_of_month, uint8_t
        , (d01, = 0)
        , (d02, = 1)
        , (d03, = 2)
        , (d04, = 3)
        , (d05, = 4)
        , (d06, = 5)
        , (d07, = 6)
        , (d08, = 7)
        , (d09, = 8)
        , (d10, = 9)
        , (d11, = 10)
        , (d12, = 11)
        , (d13, = 12)
        , (d14, = 13)
        , (d15, = 14)
        , (d16, = 15)
        , (d17, = 16)
        , (d18, = 17)
        , (d19, = 18)
        , (d20, = 19)
        , (d21, = 20)
        , (d22, = 21)
        , (d23, = 22)
        , (d24, = 23)
        , (d25, = 24)
        , (d26, = 25)
        , (d27, = 26)
        , (d28, = 27)
        , (d29, = 28)
        , (d30, = 29)
        , (d31, = 30)
    );


    UNS_RENUM(hour, uint8_t
        , (h00, = 0)
        , (h01, = 1)
        , (h02, = 2)
        , (h03, = 3)
        , (h04, = 4)
        , (h05, = 5)
        , (h06, = 6)
        , (h07, = 7)
        , (h08, = 8)
        , (h09, = 9)
        , (h10, = 10)
        , (h11, = 11)
        , (h12, = 12)
        , (h13, = 13)
        , (h14, = 14)
        , (h15, = 15)
        , (h16, = 16)
        , (h17, = 17)
        , (h18, = 18)
        , (h19, = 19)
        , (h20, = 20)
        , (h21, = 21)
        , (h22, = 22)
        , (h23, = 23)
    );


    UNS_RENUM(minute, uint8_t
        , (m00, = 0)
        , (m01, = 1)
        , (m02, = 2)
        , (m03, = 3)
        , (m04, = 4)
        , (m05, = 5)
        , (m06, = 6)
        , (m07, = 7)
        , (m08, = 8)
        , (m09, = 9)
        , (m10, = 10)
        , (m11, = 11)
        , (m12, = 12)
        , (m13, = 13)
        , (m14, = 14)
        , (m15, = 15)
        , (m16, = 16)
        , (m17, = 17)
        , (m18, = 18)
        , (m19, = 19)
        , (m20, = 20)
        , (m21, = 21)
        , (m22, = 22)
        , (m23, = 23)
        , (m24, = 24)
        , (m25, = 25)
        , (m26, = 26)
        , (m27, = 27)
        , (m28, = 28)
        , (m29, = 29)
        , (m30, = 30)
        , (m31, = 31)
        , (m32, = 32)
        , (m33, = 33)
        , (m34, = 34)
        , (m35, = 35)
        , (m36, = 36)
        , (m37, = 37)
        , (m38, = 38)
        , (m39, = 39)
        , (m40, = 40)
        , (m41, = 41)
        , (m42, = 42)
        , (m43, = 43)
        , (m44, = 44)
        , (m45, = 45)
        , (m46, = 46)
        , (m47, = 47)
        , (m48, = 48)
        , (m49, = 49)
        , (m51, = 51)
        , (m52, = 52)
        , (m53, = 53)
        , (m54, = 54)
        , (m55, = 55)
        , (m56, = 56)
        , (m57, = 57)
        , (m58, = 58)
        , (m59, = 59)
    );


    UNS_RENUM(second, uint8_t
        , (s00, = 0)
        , (s01, = 1)
        , (s02, = 2)
        , (s03, = 3)
        , (s04, = 4)
        , (s05, = 5)
        , (s06, = 6)
        , (s07, = 7)
        , (s08, = 8)
        , (s09, = 9)
        , (s10, = 10)
        , (s11, = 11)
        , (s12, = 12)
        , (s13, = 13)
        , (s14, = 14)
        , (s15, = 15)
        , (s16, = 16)
        , (s17, = 17)
        , (s18, = 18)
        , (s19, = 19)
        , (s20, = 20)
        , (s21, = 21)
        , (s22, = 22)
        , (s23, = 23)
        , (s24, = 24)
        , (s25, = 25)
        , (s26, = 26)
        , (s27, = 27)
        , (s28, = 28)
        , (s29, = 29)
        , (s30, = 30)
        , (s31, = 31)
        , (s32, = 32)
        , (s33, = 33)
        , (s34, = 34)
        , (s35, = 35)
        , (s36, = 36)
        , (s37, = 37)
        , (s38, = 38)
        , (s39, = 39)
        , (s40, = 40)
        , (s41, = 41)
        , (s42, = 42)
        , (s43, = 43)
        , (s44, = 44)
        , (s45, = 45)
        , (s46, = 46)
        , (s47, = 47)
        , (s48, = 48)
        , (s49, = 49)
        , (s51, = 51)
        , (s52, = 52)
        , (s53, = 53)
        , (s54, = 54)
        , (s55, = 55)
        , (s56, = 56)
        , (s57, = 57)
        , (s58, = 58)
        , (s59, = 59)
    );


    using clock_type = ::std::chrono::system_clock;
    using time_point = typename ::uns::calendar::gregorian::clock_type::time_point;


    class datetime {
    public:
        int year = 1970;
        ::uns::calendar::gregorian::month month = ::uns::calendar::gregorian::month::january;
        ::uns::calendar::gregorian::day_of_month day = ::uns::calendar::gregorian::day_of_month::d01;
        ::uns::calendar::gregorian::hour hours = ::uns::calendar::gregorian::hour::h00;
        ::uns::calendar::gregorian::minute minutes = ::uns::calendar::gregorian::minute::m00;
        ::uns::calendar::gregorian::second seconds = ::uns::calendar::gregorian::second::s00;
        int nanoseconds = 0;
    public:
        datetime() noexcept = default;
        explicit datetime(const ::uns::calendar::gregorian::time_point& TimePoint) noexcept;
        datetime(const ::uns::calendar::gregorian::datetime&) noexcept = default;
        ::uns::calendar::gregorian::datetime& operator=(const ::uns::calendar::gregorian::datetime&) noexcept = default;
        datetime(::uns::calendar::gregorian::datetime&&) noexcept = default;
        ::uns::calendar::gregorian::datetime& operator=(::uns::calendar::gregorian::datetime&&) noexcept = default;
        ~datetime() noexcept = default;
    public:
        operator time_point() const;
    public:
        bool ok() const noexcept;
    };
};

#endif
