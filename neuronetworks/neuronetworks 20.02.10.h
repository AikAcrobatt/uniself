...
namespace uns {
	namespace nn {
		...
		public:
			...
			activator& operator=(const activator& a) { return *this; };
			activator(activator&& a) : {};
			activator& operator=(activator&& a) { return *this; };
			~activator() {};
			virtual uns::nn::string_t Type() const { return uns::unified_cast<string_t>(L"Zero"); };
			virtual signal_t operator()(signal_t S, std::vector<signal_t>& params) const { return 0; };
	...
	namespace nn {
		...
		public:
			...
			reverse_neuron_representation<signal_t>& operator=(const reverse_neuron_representation<signal_t>& repr) {
				this = uns::unified_cast<neuron_representation<signal_t>>(repr);
				_r = repr._r;
				_s = repr._s;
				_links = repr._links;
			...
			reverse_neuron_representation<signal_t>& operator=(reverse_neuron_representation<signal_t>&& repr) {
				this = std::move(uns::unified_cast<neuron_representation<signal_t>>(repr));
				_r = repr._r;
				_s = repr._s;
	...
	namespace nn {
		...
		inline void nonrecursive_reverse_network<signal_t>::Construct(const uns::nn::input_data_object_interface<signal_t>* const _ido) {
			...
			for (auto output : base_t::outputs) {
				...
				if (output >= 0 && output < base_t::neurons.size()) base_t::neurons[output]._inputs.push_back(_inputs[_inputs.size() - 1]);
			};
			...
			for (auto neuron = base_t::neurons.rbegin(); neuron != base_t::neurons.rend(); neuron++) {
				link_number = 0;
				for (auto& link : neuron.links) {
					dynamic_cast<neuron_t>(link.first)->_links.push_back(std::pair<neuron_t*, size_t>(this, link_number++));
				};
			};
		};
