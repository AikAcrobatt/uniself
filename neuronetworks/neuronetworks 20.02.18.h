...
namespace uns {
	namespace nn {
		...
		public:
			//using represented_class = nonrecursive_neuron<signal_t>;
		public:
			signal_t r = 0;
			signal_t s = 0;
		...
		public:
			//using represented_class = nonrecursive_reverse_neuron<signal_t>;
		public:
			signal_t _r = 0;
			signal_t _s = 0;
		...
		public:
			...
			virtual bool Set(const string_t& str) override {
				...
				size_t seeker = 0;
				if (!uns::string::SeekerSet<string_t>(str, seeker, L"[_(", false, -1)) return false;
		...
		public:
			string_t activator_type = L"Zero";
			string_t collector_type = L"Zero";
			...
			string_t String(string_t delimiter = L"\n") const {
				string_t res = L"LAYER " + activator_type + L"__" + collector_type + L":" + delimiter;
			...
			bool Set(const string_t& str, string_t delimiter) {
				size_t seeker = 0;
				if (!uns::string::SeekerSet<string_t>(str, seeker, L"LAYER ", false, -1)) return false;
				string_t temp = L"";
				if (!uns::string::SeekerRead<string_t>(str, seeker, temp, L"__", false, -1, L":")) return false;
				else activator_type = temp;
				if (!uns::string::SeekerRead<string_t>(str, seeker, temp, L":" + delimiter, false, -1, L":LAYER")) return false;
				else collector_type = temp;

				neurons.clear();
				do {
					if (!uns::string::SeekerRead<string_t>(str, seeker, temp, delimiter, false, -1, L":LAYER")) break;
		...
		public:
			...
			network_representation(const network_representation<neuron_representation_t>& repr) : layers(repr.layers), outputs(repr.outputs) {};
			network_representation<neuron_representation_t>& operator=(const network_representation<neuron_representation_t>& repr) { layers = repr.layers; outputs = repr.outputs;  return *this; };
			network_representation(network_representation<neuron_representation_t>&& repr) noexcept : layers(std::move(repr.layers)), outputs(std::move(repr.outputs)) {};
			network_representation<neuron_representation_t>& operator=(network_representation<neuron_representation_t>&& repr) { layers = std::move(repr.layers); outputs = std::move(repr.outputs); return *this; };
			~network_representation() noexcept {};
			string_t String(string_t delimiter = L"\n") const {
				string_t res = L"NETWORK:" + delimiter;
				for (auto layer : layers)
					res += layer.String() + L";" +delimiter;
				res += L"OUTPUTS:";
			...
			bool Set(const string_t& str, string_t delimiter) {
				...
				layers.clear();
				do {
					if (!uns::string::SeekerRead<string_t>(str, seeker, temp, L";" + delimiter, false, -1, L"OUTPUTS:")) break;
		...
		public:
			...
			//virtual neuron_representation<signal_t> Represent_() const;
		...
		/*template<typename signal_t>
		inline neuron_representation<signal_t> uns::nn::nonrecursive_neuron<signal_t>::Represent_() const {
			return Represent();
		};*/
		...
		public:
			...
			//virtual uns::nn::reverse_neuron_representation<signal_t> Represent_() const override;
		...
		/*template<typename signal_t>
		inline uns::nn::reverse_neuron_representation<signal_t> uns::nn::nonrecursive_reverse_neuron<signal_t>::Represent_() const {
			return _Represent();
		};*/
		...
		protected:
			using network_iter_t = typename std::vector<neuron_t*>::const_iterator;

			std::vector<neuron_t*> neurons;
			std::vector<std::pair<network_iter_t, network_iter_t>> layer_frames;
		public:
		...
		public:
			...
			//template<template<typename neuron_signal_t>class neuron_representation_t> network_representation<neuron_representation_t<signal_t>> Represent_();
		...
		void nonrecursive_base_network<neuron_t>::Set(const uns::nn::network_representation<neuron_representation<signal_t>>& repr, const uns::nn::input_data_object_interface<signal_t>& ido) {
			...
			size_t layer_number = 0;
			for (auto layer : repr.layers) {
				layer_frames.push_back(std::pair<network_iter_t, network_iter_t>(neurons.cbegin() + neuron_number, neurons.cbegin() + neuron_number + 1));
				for (auto neuron : layer.neurons) {
				...
				layer_frames[layer_number].second = neurons.cbegin() + neuron_number;
				layer_number++;
			};
		...
		inline network_representation<neuron_representation<typename neuron_t::neuron_signal_t>> nonrecursive_base_network<neuron_t>::Represent() {
			...
			res.layers.resize(layer_frames.size());
			size_t layer_number = 0;
			for (auto layer_frame : nonrecursive_base_network<neuron_t>::layer_frames) {
				for (auto neuron_ptr = layer_frame.first; neuron_ptr != layer_frame.second; neuron_ptr++)
					res.layers[layer_number].neurons.push_back((*neuron_ptr)->Represent());
				if (layer_frame.first != layer_frame.second) {
					res.layers[layer_number].activator_type = (*(layer_frame.first))->F->Type();
					res.layers[layer_number].collector_type = (*(layer_frame.first))->S->Type();
				};
				layer_number++;
			};
		...
		/*template<class neuron_t> 
			template<template<typename neuron_signal_t>class neuron_representation_t>
				network_representation<neuron_representation_t<typename nonrecursive_base_network<neuron_t>::signal_t>> nonrecursive_base_network<neuron_t>::Represent_() 
		{
			network_representation<neuron_representation_t<signal_t>> res;

			res.layers.resize(layer_frames.size());
			size_t layer_number = 0;
			for (auto layer_frame : layer_frames) {
				for (auto neuron_ptr = layer_frame.first; neuron_ptr != layer_frame.second; neuron_ptr++)
					res.layers[layer_number].neurons.push_back((dynamic_cast<typename neuron_representation_t<signal_t>::represented_class*>(*neuron_ptr)->Represent_()));
				if (layer_frame.first != layer_frame.second) {
					res.layers[layer_number].activator_type = (*(layer_frame.first))->F->Type();
					res.layers[layer_number].collector_type = (*(layer_frame.first))->S->Type();
				};
		...
		inline network_representation<reverse_neuron_representation<signal_t>> nonrecursive_reverse_network<signal_t>::_Represent() {
			...
			res.layers.resize(base_t::layer_frames.size());
			size_t layer_number = 0;
			for (auto layer_frame : base_t::layer_frames) {
				for (auto neuron_ptr = layer_frame.first; neuron_ptr != layer_frame.second; neuron_ptr++)
					res.layers[layer_number].neurons.push_back((*neuron_ptr)->Represent());
				if (layer_frame.first != layer_frame.second) {
					res.layers[layer_number].activator_type = (*(layer_frame.first))->F->Type();
