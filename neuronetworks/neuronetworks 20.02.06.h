...
namespace uns {
	...
	namespace nn {
		...
		protected:
			string_t type = L"";
			signal_t(*main_functor) (std::vector<std::pair<neuron_interface<signal_t>*, signal_t>>&, std::vector<signal_t>&);
			signal_t(*dS_dr) (size_t, const std::vector<std::pair<neuron_interface<signal_t>*, signal_t>>&, const std::vector<signal_t>&);
			signal_t(*dS_dw) (size_t, const std::vector<std::pair<neuron_interface<signal_t>*, signal_t>>&, const std::vector<signal_t>&);
			signal_t(*dS_dp) (size_t, const std::vector<std::pair<neuron_interface<signal_t>*, signal_t>>&, const std::vector<signal_t>&);
		public:
			collector() :
				type(L"Zero"),
				main_functor([](std::vector<std::pair<neuron_interface<signal_t>*, signal_t>>& l, std::vector<signal_t>& p) -> signal_t { return 0; }),
				dS_dr([](size_t index, const std::vector<std::pair<neuron_interface<signal_t>*, signal_t>>& l, const std::vector<signal_t>& p) -> signal_t { return 0; }),
				dS_dw([](size_t index, const std::vector<std::pair<neuron_interface<signal_t>*, signal_t>>& l, const std::vector<signal_t>& p) -> signal_t { return 0; }),
				dS_dp([](size_t index, const std::vector<std::pair<neuron_interface<signal_t>*, signal_t>>& l, const std::vector<signal_t>& p) -> signal_t { return 0; })
			{};
			collector(
				const string_t& collector_type,
				signal_t(* const main) (std::vector<std::pair<neuron_interface<signal_t>*, signal_t>>&, std::vector<signal_t>&),
				signal_t(* const _dr) (size_t index, const std::vector<std::pair<neuron_interface<signal_t>*, signal_t>>&, const std::vector<signal_t>&),
				signal_t(* const _dw) (size_t index, const std::vector<std::pair<neuron_interface<signal_t>*, signal_t>>&, const std::vector<signal_t>&),
				signal_t(* const _dp) (size_t index, const std::vector<std::pair<neuron_interface<signal_t>*, signal_t>>&, const std::vector<signal_t>&)
			) : type(collector_type), main_functor(main), dS_dr(_dr), dS_dw(_dw), dS_dp(_dp) {};
			collector(const collector<signal_t>& c) : type(c.type), main_functor(c.main_functor), dS_dr(c.dS_dr), dS_dw(c.dS_dw), dS_dp(c.dS_dp) {};
			collector<signal_t>& operator=(const collector<signal_t>& c) { type = c.type; main_functor = c.main_functor; dS_dr = c.dS_dr; dS_dw = c.dS_dw; dS_dp = c.dS_dp; return *this; };
			collector(collector<signal_t>&& c) : type(std::move(c.type)), main_functor(c.main_functor), dS_dr(c.dS_dr), dS_dw(c.dS_dw), dS_dp(c.dS_dp) {};
			collector<signal_t>& operator=(collector<signal_t>&& c) { type = std::move(c.type); main_functor = c.main_functor; dS_dr = c.dS_dr; dS_dw = c.dS_dw; dS_dp = c.dS_dp;  return *this; };
			~collector() {};
			uns::nn::string_t Type() const { return type; };
			signal_t operator()(std::vector<std::pair<neuron_interface<signal_t>*, signal_t>>& links, std::vector<signal_t>& params) const { return main_functor(links, params); };
			signal_t _dr(size_t index, const std::vector<std::pair<neuron_interface<signal_t>*, signal_t>>& links, const std::vector<signal_t>& params) const { return dS_dr(index, links, params); };
			signal_t _dw(size_t index, const std::vector<std::pair<neuron_interface<signal_t>*, signal_t>>& links, const std::vector<signal_t>& params) const { return dS_dw(index, links, params); };
			signal_t _dp(size_t index, const std::vector<std::pair<neuron_interface<signal_t>*, signal_t>>& links, const std::vector<signal_t>& params) const { return dS_dp(index, links, params); };
		};
	...
	namespace nn {
		...
		public:
			...
			neuron_representation(const neuron_representation<signal_t>& repr) : activator_type(repr.activator_type), collector_type(repr.collector_type), r(repr.r), s(repr.s), links(repr.links), params(repr.params) {};
			...
			neuron_representation(neuron_representation<signal_t>&& repr) : activator_type(std::move(repr.activator_type)), collector_type(std::move(repr.collector_type)), r(repr.r), s(repr.s), links(std::move(repr.links)), params(std::move(repr.params)) {};
		...
		template<typename signal_t, template<typename neuron_signal_t> class neuron_t>
		class nonrecursive_network;
		template<typename signal_t>
		class nonrecursive_neuron : public neuron_interface<signal_t> {
			friend nonrecursive_network<signal_t, nonrecursive_neuron<signal_t>>;
		protected:
		...
		public:
			...
			reverse_neuron_representation(const neuron_representation<signal_t>& represent) : neuron_representation(represent.activator_type, represent.collector_type, represent.r, represent.s, represent.links, represent.params) { _links.clear(); };
			reverse_neuron_representation(const string_t& str) { Set(str); };
			reverse_neuron_representation(const reverse_neuron_representation<signal_t>& repr) : neuron_representation(repr), _r(repr._r), _s(repr._s), _links(repr._links), is_learning(repr.is_learning), not_dropout(repr.not_dropout) {};
			reverse_neuron_representation<signal_t>& operator=(const reverse_neuron_representation<signal_t>& repr) {
				neuron_representation<signal_t>::activator_type = repr.activator_type;
				neuron_representation<signal_t>::collector_type = repr.collector_type;
				neuron_representation<signal_t>::links = repr.links;
				neuron_representation<signal_t>::params = repr.params;
				neuron_representation<signal_t>::r = repr.r;
				neuron_representation<signal_t>::s = repr.s;
				_r = repr._r;
				_s = repr._s;
				_links = repr._links;
			...
			reverse_neuron_representation(reverse_neuron_representation<signal_t>&& repr) : neuron_representation(std::move(repr)), _r(repr._r), _s(repr._s), _links(std::move(repr._links)), is_learning(repr.is_learning), not_dropout(repr.not_dropout) {};
			reverse_neuron_representation<signal_t>& operator=(reverse_neuron_representation<signal_t>&& repr) {
				neuron_representation<signal_t>::activator_type = std::move(repr.activator_type);
				neuron_representation<signal_t>::collector_type = std::move(repr.collector_type);
				neuron_representation<signal_t>::links = std::move(repr.links);
				neuron_representation<signal_t>::params = std::move(repr.params);
				neuron_representation<signal_t>::r = repr.r;
				neuron_representation<signal_t>::s = repr.s;
				_r = repr._r;
				_s = repr._s;
			...
			virtual string_t String() const override {
				...
				string_t res = neuron_representation<signal_t>::String();
				res += L"[Reverse_N(";
			...
			signal_t Dropout() const { return 1.0 - not_dropout; };
		...
		class nonrecursive_reverse_neuron : public nonrecursive_neuron<signal_t> {
			friend nonrecursive_reverse_network<signal_t, nonrecursive_reverse_neuron<signal_t>>;
		protected:
			signal_t _r = 0;
			signal_t _s = 0;
		...
		public:
			...
			~nonrecursive_reverse_neuron() {};
			uns::nn::reverse_neuron_representation<signal_t> Represent() const;
			...
			signal_t dS_dr(size_t index) const { return nonrecursive_neuron<signal_t>::S._dr(index, nonrecursive_neuron<signal_t>::links, nonrecursive_neuron<signal_t>::params); };
			signal_t dS_dw(size_t index) const { return nonrecursive_neuron<signal_t>::S._dw(index, nonrecursive_neuron<signal_t>::links, nonrecursive_neuron<signal_t>::params); };
			signal_t dS_dp(size_t index) const { return nonrecursive_neuron<signal_t>::S._dp(index, nonrecursive_neuron<signal_t>::links, nonrecursive_neuron<signal_t>::params); };
			signal_t dF_dS() const { return nonrecursive_neuron<signal_t>::F._dS(nonrecursive_neuron<signal_t>::s, nonrecursive_neuron<signal_t>::params); };
			signal_t dF_dp(size_t index) const { return nonrecursive_neuron<signal_t>::F._dp(index, nonrecursive_neuron<signal_t>::s, nonrecursive_neuron<signal_t>::params); };
		};

		template<typename signal_t>
		inline uns::nn::reverse_neuron_representation<signal_t> uns::nn::nonrecursive_reverse_neuron<signal_t>::Represent() const {
			uns::nn::reverse_neuron_representation<signal_t> res(uns::nn::nonrecursive_neuron::Represent());
			res._r = _r;
			res._s = _s;
		...
		public:
			input_neuron() {};
			input_neuron(size_t num_of_input, const input_data_object_interface<signal_t>* const ido) { Set(num_of_input, ido); };
			...
			void Set(size_t num_of_input, const input_data_object_interface<signal_t>* const ido) {
		...
		public:
			...
			network_representation(network_representation<neuron_representation_t>&& repr) : neurons(std::move(repr.neurons)), outputs(std::move(repr.outputs)) {};
	...
	namespace nn {
		...
		/*template<typename signal_t>
		class nonrecursive_network : public network_interface<signal_t> {
		protected:
			std::vector<nonrecursive_neuron<signal_t>*> neurons;
		public:
			nonrecursive_network() {};
			nonrecursive_network(const network_representation<neuron_representation<signal_t>>& repr, const uns::nn::input_data_object_interface<signal_t>* const ido);
			nonrecursive_network(const nonrecursive_network<signal_t>& net) = delete;
			nonrecursive_network<signal_t> operator=(const nonrecursive_network<signal_t>& net) = delete;
			nonrecursive_network(nonrecursive_network<signal_t>&& net) = delete;
			nonrecursive_network<signal_t> operator=(nonrecursive_network<signal_t>&& net) = delete;
			~nonrecursive_network() {
				for (auto& neuron : neurons)
					delete neuron;
				for (auto& input : network_interface<signal_t>::inputs)
					delete input;
			};
			...
			signal_t O(size_t index) const { return network_interface<signal_t>::outputs[index]->R(); };
			signal_t I(size_t index) const { return network_interface<signal_t>::inputs[index]->R(); };
			size_t Neurons() const { return neurons.size(); };
			size_t Outputs() const { return network_interface<signal_t>::outputs.size(); };
			size_t Inputs() const { return network_interface<signal_t>::inputs.size(); };
		protected:
			void Set(const uns::nn::network_representation<neuron_representation<signal_t>>& repr, const uns::nn::input_data_object_interface<signal_t>* const ido);
		};

		template<typename signal_t>
		inline void nonrecursive_network<signal_t>::Set(const uns::nn::network_representation<neuron_representation<signal_t>>& repr, const uns::nn::input_data_object_interface<signal_t>* const ido) {
			network_interface<signal_t>::inputs.clear();
			neurons.clear();
			network_interface<signal_t>::outputs.clear();

			nonrecursive_neuron<signal_t>* next_neuron = nullptr;
			for (auto neuron : repr.neurons) {
				next_neuron = new nonrecursive_neuron<signal_t>();
				...
				for (auto link : neuron.links) {
					...
					if (link.first < 0) {
						...
						if (input_neurons_numbers[input_number] == empty_meaning) {
							network_interface<signal_t>::inputs.push_back(new input_neuron<signal_t>(input_number, ido));
							input_neurons_numbers[input_number] = uns::unified_cast<int>(network_interface<signal_t>::inputs.size() - 1);
						};
						if (input_neurons_numbers[input_number] > empty_meaning)
							neurons[neuron_number]->links.push_back(std::pair<input_neuron<signal_t>*, signal_t>(network_interface<signal_t>::inputs[uns::unified_cast<size_t>(input_neurons_numbers[input_number])], link.second));
					}
					else if (link.first < neurons.size())
						neurons[neuron_number]->links.push_back(std::pair<nonrecursive_neuron<signal_t>*, signal_t>(neurons[uns::unified_cast<size_t>(link.first)], link.second));
				};
				neuron_number++;
			};
			...
			for (auto output : repr.outputs)
				if (output < repr.neurons.size())
					network_interface<signal_t>::outputs.push_back(neurons[output]);
		};

		template<typename signal_t>
		inline network_representation<neuron_representation<signal_t>> nonrecursive_network<signal_t>::Represent() {
		...
		class nonrecursive_network : public network_interface<signal_t> {
		protected:
			std::vector<neuron_t<signal_t>*> neurons;
		public:
			nonrecursive_network() {};
			nonrecursive_network(const network_representation<neuron_representation<signal_t>>& repr, const uns::nn::input_data_object_interface<signal_t>* const ido) { Set(repr, ido); };
			nonrecursive_network(const nonrecursive_network<signal_t, neuron_t>& net) = delete;
			nonrecursive_network<signal_t, neuron_t> operator=(const nonrecursive_network<signal_t, neuron_t>& net) = delete;
			nonrecursive_network(nonrecursive_network<signal_t, neuron_t>&& net) = delete;
			nonrecursive_network<signal_t, neuron_t> operator=(nonrecursive_network<signal_t, neuron_t>&& net) = delete;
			~nonrecursive_network() {
				for (auto& neuron : neurons)
					delete neuron;
				for (auto& input : network_interface<signal_t>::inputs)
		...
		template<typename signal_t, template<typename neuron_signal_t> class neuron_t>
		inline void nonrecursive_network<signal_t, neuron_t>::Set(const uns::nn::network_representation<neuron_representation<signal_t>>& repr, const uns::nn::input_data_object_interface<signal_t>* const ido) {
			network_interface<signal_t>::inputs.clear();
			neurons.clear();
			network_interface<signal_t>::outputs.clear();

			neuron_t<signal_t>* next_neuron = nullptr;
			for (auto neuron : repr.neurons) {
				next_neuron = new neuron_t<signal_t>();
				next_neuron->F = uns::unified_cast<uns::nn::activator>(neuron.activator_type);
				next_neuron->S = uns::unified_cast<uns::nn::collector>(neuron.collector_type);
				next_neuron->r = neuron.r;
				next_neuron->s = neuron.s;
		...
		template<typename signal_t, template<typename neuron_signal_t> class neuron_t>
		inline network_representation<neuron_representation<signal_t>> nonrecursive_network<signal_t, neuron_t>::Represent() {
			network_representation<neuron_representation<signal_t>> res;
			for (auto neuron : neurons)
				res.neurons.push_back(neuron->Represent());
			for (auto output : network_interface<signal_t>::outputs)
		...
		/*template<typename signal_t>
		class nonrecursive_reverse_network : public network_interface<signal_t> {
		protected:
			std::vector<nonrecursive_reverse_neuron<signal_t>*> neurons;
			std::vector<input_neuron<signal_t>*> _inputs;
		public:
			nonrecursive_reverse_network() {};
			nonrecursive_reverse_network(const network_representation<neuron_representation<signal_t>>& repr, const uns::nn::input_data_object_interface<signal_t>* const ido, const uns::nn::input_data_object_interface<signal_t>* const _ido);
			...
			network_representation<neuron_representation<signal_t>> Represent();
			void React(const input_data_object_interface<signal_t>& const ido) {
				for (auto& input : network_interface<signal_t>::inputs) {
					input->Set(input->Index(), &ido);
					input->React();
				};
		...
		template<typename signal_t, template<typename neuron_signal_t> class neuron_t>
		class nonrecursive_reverse_network : public nonrecursive_network<signal_t, neuron_t> {
		protected:
			std::vector<input_neuron<signal_t>*> _inputs;
		public:
			nonrecursive_reverse_network() {};
			nonrecursive_reverse_network(const network_representation<neuron_representation<signal_t>>& repr, const uns::nn::input_data_object_interface<signal_t>* const ido, const uns::nn::input_data_object_interface<signal_t>* const _ido) {
				nonrecursive_network<signal_t, neuron_t>::Set(repr, ido);
				_Set(repr, _ido);
				for (auto& _input : _inputs) {
					_input->Set(_input->Index(), &_ido);
				};
			};
			nonrecursive_reverse_network(const nonrecursive_reverse_network<signal_t, neuron_t>& net) = delete;
			nonrecursive_reverse_network<signal_t> operator=(const nonrecursive_reverse_network<signal_t, neuron_t>& net) = delete;
			nonrecursive_reverse_network(nonrecursive_reverse_network<signal_t, neuron_t>&& net) = delete;
			nonrecursive_reverse_network<signal_t> operator=(nonrecursive_reverse_network<signal_t, neuron_t>&& net) = delete;
			~nonrecursive_reverse_network() {
				for (auto& neuron : nonrecursive_network<signal_t, neuron_t>::neurons)
					delete neuron;
				for (auto& input : network_interface<signal_t>::inputs)
					delete input;
				for (auto& _input : _inputs)
			...
			void _React(const input_data_object_interface<signal_t>& _ido) {
				...
				for (auto neuron = nonrecursive_network<signal_t, neuron_t>::neurons.rbegin(); neuron != nonrecursive_network<signal_t, neuron_t>::neurons.rend(); neuron++) {
					(*neuron)->_Collect();
					(*neuron)->_React();
				};
			};
			...
			signal_t _R(size_t index) const { return nonrecursive_network<signal_t, neuron_t>::neurons[index]->_R(); };
			signal_t dS_dw(size_t neuron_idx, size_t weight_idx) const { return nonrecursive_network<signal_t, neuron_t>::neurons[neuron_idx]->dS_dw(weight_idx); };
			signal_t dS_dp(size_t neuron_idx, size_t param_idx) const { return nonrecursive_network<signal_t, neuron_t>::neurons[neuron_idx]->dS_dp(param_idx); };
			signal_t dF_dp(size_t neuron_idx, size_t param_idx) const { return nonrecursive_network<signal_t, neuron_t>::neurons[neuron_idx]->dF_dp(param_idx); };
		protected:
			void _Set(const uns::nn::network_representation<neuron_representation<signal_t>>& repr, const uns::nn::input_data_object_interface<signal_t>* const ido);
		public:
			void Construct(const uns::nn::input_data_object_interface<signal_t>* const _ido);
		};
