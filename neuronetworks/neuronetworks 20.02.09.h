...
namespace uns {
	...
	namespace nn {
		...
		template<typename signal_t, class neuron_t>
		...
		inline uns::nn::reverse_neuron_representation<signal_t> uns::nn::nonrecursive_reverse_neuron<signal_t>::Represent() const {
			uns::nn::reverse_neuron_representation<signal_t> res(uns::nn::nonrecursive_neuron<signal_t>::Represent());
			res._r = _r;
			res._s = _s;
	...
	namespace nn {
		...
		template<typename signal_t>
		class nonrecursive_reverse_network : public nonrecursive_network<signal_t, uns::nn::nonrecursive_reverse_neuron<signal_t>> {
		protected:
			std::vector<input_neuron<signal_t>*> _inputs;
			using neuron_t = uns::nn::nonrecursive_reverse_neuron<signal_t>;
			using base_t = nonrecursive_network<signal_t, neuron_t>;
		...
		public:
			...
			void _React(const input_data_object_interface<signal_t>& _ido) {
				...
				for (auto neuron = base_t::neurons.rbegin(); neuron != base_t::neurons.rend(); neuron++) {
					(*neuron)->_Collect();
					(*neuron)->_React();
				};
			};
		...
		template<typename signal_t>
		inline void nonrecursive_reverse_network<signal_t>::Construct(const uns::nn::input_data_object_interface<signal_t>* const _ido) {
			_inputs.clear();

			size_t output_number = 0;
			for (auto output : base_t::outputs) {
				_inputs.push_back(new input_neuron<signal_t>(output_number++, _ido));
				if (output >= 0 && output < base_t::neurons.size()) base_t::neurons[output]._links.push_back(std::pair<neuron_t*, size_t>(_inputs[_inputs.size() - 1], 0));
			};
			...
			for (auto neuron = base_t::neurons.rbegin(); neuron != base_t::neurons.rend(); neuron++) {
				link_number = 0;
				for (auto& link : neuron.links) {
					dynamic_cast<neuron_t>(links.first)->_links.push_back(std::pair<neuron_t*, size_t>(this, link_number++));
				};
			};
		};
