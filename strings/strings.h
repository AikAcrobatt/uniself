...
namespace uns::string {
	...
	typename string_t::size_type SeekerFind(
		...
		typename string_t::size_type found_sample_position = string_t::npos;
		...
		if (seeker == string_t::npos || seeker >= str.length()) return string_t::npos;
	...
	bool SeekerRead(
		...
		if (!(pos_delimiter > seeker && pos_delimiter < str.length())) return false;
		if (!(delimiter_number >= 0 && delimiter_number < delimiters.size())) return false;
